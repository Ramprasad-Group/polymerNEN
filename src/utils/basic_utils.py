...
def formatted_time():
    return datetime.now().strftime(Config.DATE_TIME_FORMAT)
...
def split_vec_data(config, clusters):
	''' Puts trainFrac fraction of clusters in trainClusters and remaining clusters in testClusters, but each
...
def split_vec_data_modified(config, clusters):
	''' Puts trainFrac fraction of clusters in trainClusters and remaining clusters in testClusters, but each
	  but in batches where each batch has testFrac fraction of clusters.'''
	...
	########################## Create trainCanopy ###################################
	trainCids = allCids[:numTrain]
	trainClusters = {cid: list(clusters[cid].values()) for cid in trainCids}
	trainPoints = {}
	# pid = 0
	for cid in trainClusters:
		for pid, point in clusters[cid].items():
			trainPoints[pid] = (point, cid)
			# pid += 1
	...
	if numDev > 0:
		devCids = allCids[numTrain:numTrain + numDev]
		devClusters = {cid: list(clusters[cid].values()) for cid in devCids}
		devPoints = {}
		#pid = 0
		for cid in devClusters:
			for pid, point in clusters[cid].items():
	...
	while testStartCid < len(allCids):
		...
		testCids = allCids[testStartCid:testEndCid]
		testClusters = {cid: list(clusters[cid].values()) for cid in testCids}
		#pid = 0
		testPoints = {}
		for cid in testClusters:
			for pid, point in clusters[cid].items():
...
def read_clusters_synth(filename):
	...
	:return:
	clusters: Dictionary with key as clusterIds, and value as list of points in that cluster
	'''
	clusters	= {}
	with open(filename,"r") as reader:
		for line in reader:
			...
			cid		 	= float(splitLine[2]) # 3rd value contain cluster of this point
...
def read_clusters_pid(filename):
	'''
	...
	with open(filename, "r") as reader:
		for line in reader:
			...
			clusters[cid][int(pointId)]=point
...
def extract_threshold_values(config):
	'''Extracts threshold values evaluated on dev set found in log file of already trained model '''
	with open(path.join(config.trainResultDir, config.trainLogFile), 'r') as f:
		file_text = f.read()

	target_text = re.findall('Using dev thresholdVals:{.*}', file_text)
	print(target_text)
	target_dict = re.findall('{.*}', target_text[-1])
	threshDict = json.loads(target_dict[0].replace('\'', '"'))
	return threshDict


def save_predicted_values(y_pred, points, folder, method, mode, file_name='predicted_clusters'):
	with open(path.join(folder, f'{file_name}_{method}_{mode}'), 'w') as f:
		for i, pid in enumerate(points.keys()):
			f.write(f'{pid},{y_pred[i]}')
			f.write('\n')