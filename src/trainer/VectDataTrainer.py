...
from utils.basic_utils import create_logger, load_vec_data_splits, calc_batch_size, read_clusters_pid
...
class VectDataTrainer(BaseTrainer):
	...
	def __init__(self,config):
		...
		if self.config.mode == 'test':
			testClusters = read_clusters_pid(self.config.clusterFile)
			testPoints = {}
			# pid = 0
			for cid in testClusters:
				for pid, point in testClusters[cid].items():
					testPoints[pid] = (point, cid)
					# pid += 1
			testClusters = {cid: list(testClusters[cid].values()) for cid in testClusters.keys()}
			self.testCanopies = [{"clusters": testClusters, "points": testPoints}] # Cluster values here are garbage as we dont know the actual values for our test clusters
	...
	def plotAndWriteData(self, allLosses, allScores):
		...
		if self.config.makeScorePlots:
			plot_scores(allLosses=allLosses, allScores=allScores, currResultDir=self.config.resultDir, xlabel="Epoch", config=self.config)
		else:
			# Avoid plotting scores, just plot losses
			plot_scores(allLosses=allLosses, allScores={"train":{}, "test":{}, "dev":{}}, currResultDir=self.config.resultDir, xlabel="Epoch", config=self.config)
	...
	def calcOverallHACLoss(self, canopy, linkAlpha):
		...
		pidToGtCluster = {}
		index2pid = {}
		for i, pid in enumerate(canopy["points"]):  # TODO: Precompute this if this takes time
		...
		posLinkages, negLinkages = runHAC_torch_allEdges_faces(origDistMat=distMat_NP, origTorchDistMat=torchDistMat,
														 ...
														 scaleDist=self.config.scaleDist, getBestImpure=False, index2pid_dict=index2pid)
