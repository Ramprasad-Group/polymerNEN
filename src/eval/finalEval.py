...
from utils.basic_utils import extract_threshold_values
...
def run_final_eval(trainer):
	...
	elif trainer.config.mode=='test':
		#Get threshold values from log file pre-evaluated
		threshDict = extract_threshold_values(trainer.config)
		# Scale all thresholds down to account for difference in density of train and test points
		for method, threshold in threshDict.items():
			threshDict[method] = threshold*trainer.config.thresh_scaling_factor
		trainer.logger.info("Using dev thresholdVals taken from file:{}".format(threshDict))
		eval_all_test_data(trainer, threshDict)
		#pass
...
def eval_all_test_data(trainer, threshDict):
	allScores = {"test": {}}
	...
	allScores["test"][0] = trainer.evalFunc(config=trainer.config, model=trainer.model, canopies=trainer.testCanopies,
											threshDict=threshDict,
											inferenceMethods=infMethods, metricsForEval=trainer.config.metricsForEval, mode="test")

	return allScores["test"][0]
...
def eval_all_data(trainer, threshDict, relResultDir = None):
	...
	allScores["dev"][0]   = trainer.evalFunc(config=trainer.config,model=trainer.model, canopies=trainer.devCanopies, threshDict=threshDict,
									 inferenceMethods=infMethods, metricsForEval=trainer.config.metricsForEval, mode='dev')
	...
	allScores["train"][0] = trainer.evalFunc(config=trainer.config,model=trainer.model, canopies=trainer.trainCanopies, threshDict=threshDict,
									   inferenceMethods=infMethods, metricsForEval=trainer.config.metricsForEval, mode='train')
