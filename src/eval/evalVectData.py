...
from models.parameterized_cosine import ParameterizedCosine
# from models.similarity_scaling import SimilarityScaling
...
from utils.basic_utils import save_predicted_values

perMethodMetrics = ["precision", "recall", "f1", "randIndex", "dendPurity", "nmi"]

def eval_model_vect(config, model, canopies, inferenceMethods, threshDict, metricsForEval, mode=''):
	
	metricList = []
	for method in inferenceMethods:
		for metric in perMethodMetrics:
			metricList.append(method + "_" + metric)
	if not path.isdir(path.join(config.resultDir, 'clusterLabels')):
		mkdir(path.join(config.resultDir, 'clusterLabels'))
	...
	for ctr, canopy in enumerate(canopies):
		y_true, y_pred_dict, currTestScore = perform_inference(model=model, points=canopy["points"], threshDict=threshDict,
	...
	if mode=='test' or mode=='dev' or mode=='train':
		# Save file with PID and predicted cluster_id
		save_predicted_values(y_true, canopies[0]["points"], path.join(config.resultDir, 'clusterLabels'), method='',
							  mode=mode,
							  file_name='true_clusters')
		for method in y_pred_dict.keys():
			assert len(y_pred_dict[method])==len(canopies[0]["points"])
			save_predicted_values(y_pred_dict[method], canopies[0]["points"], path.join(config.resultDir, 'clusterLabels'), method, mode)
			#pass
	end = time.time()
	# logger.info("==" * 20)
	# for metric in metricList:
...
def perform_inference(model, points, inferenceMethods, threshDict, metricsForEval, scaleDist):
	...
	elif isinstance(model, ParameterizedCosine):  # Tranform points if using cosine distance
		transformedPointList = model.transformPoints(pointList)
		linkMetric = "cosine"
	elif isinstance(model, SimilarityScaling):  # Tranform points if using cosine distance
		transformedPointList = model.transformPoints(pointList)
		linkMetric = "cosine"
	else:
	...
	#save_predicted_values(y_true, points, path.join(config.resultDir, 'clusterLabels'), mode='', method='', file_name='true_clusters')
